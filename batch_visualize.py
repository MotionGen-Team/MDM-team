...
# 配置
# -------------------------------
input_dir = r"D:\coursework\machine learning\motion-diffusion-model-main\dataset\HumanML3D\new_joint_vecs"  # 263 数据路径
video_dir = os.path.abspath('videos')
os.makedirs(video_dir, exist_ok=True)

max_samples = 10  # None=全部,可以改成你想生成的样本数
scale_factor = 5     # 放大骨架
fps = 20             # 帧率
...
for i, file in enumerate(files):
    ...
    data_263 = np.load(npy_path)
    print(f"[{i+1}/{len(files)}] Processing {file}, shape: {data_263.shape}")

    # 转 tensor
    data_263_tensor = torch.from_numpy(data_263).float()
