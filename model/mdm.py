...
class MDM(nn.Module):  # 定义主模型 MDM
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,  # 初始化基础输入参数
        ...
        self.ms_tcn_variant = kargs.get('ms_tcn_variant', None)  # 多尺度 TCN 变体名称
        ...
        if self.arch == 'trans_enc':  # 如果主干是 Transformer Encoder
            ...
            if self.use_temporal_tcn:
                # 如果指定了多尺度 TCN 变体，使用新的多尺度模块
                if self.ms_tcn_variant is not None:
                    print(f"Using MultiScaleTemporalTCN variant: {self.ms_tcn_variant}")
                    self.temporal_tcn = get_multi_scale_tcn(self.ms_tcn_variant, self.latent_dim, self.dropout)
                else:
                    # 否则使用原始的轻量级 TCN
